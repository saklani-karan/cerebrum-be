...
export function transformQuery<Query extends RequestQueryFields<Entity>, Entity extends BaseEntity>(
    ...
    const select = validateAndRetrieveFields<Entity>(fields, transformConfig) || [];
...
function validateAndRetrieveOrder<Entity extends BaseEntity>(order: string): TransformQueryOrder {
    if (!order) {
        return;
    }
